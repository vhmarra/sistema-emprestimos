...
import br.com.victor.emprestimos.dtos.SuccessResponse;
...
public class AutenticaController {
    ...
    public ResponseEntity<?> cadastraCliente(@ModelAttribute @RequestAttribute CadastraClienteRequest request) throws InvalidInputException, MessagingException {
        ...
        return ResponseEntity.status(201).body(new SuccessResponse("Cliente cadastrado com sucesso",201L));
    }
    ...
    public ResponseEntity<SuccessResponse> trocaSenha(@RequestHeader String token, @RequestHeader String novaSenha) throws Exception {
        alteraDadosService.alteraSenha(token, novaSenha);
        return ResponseEntity.ok(new SuccessResponse("Senha trocada com sucesso",200L));
    }
